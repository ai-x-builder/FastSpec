...
- `skills/spec-workflow/SKILL.md:14` lists `PRODUCT.md` and `TECH.md` as the required artifacts, but not `GATES.json`.
- `skills/spec-workflow/SKILL.md:88` and `skills/spec-workflow/SKILL.md:116` define the PRODUCT and TECH review gates.
...
- `skills/spec-loop-runner/SKILL.md` requires confirming both review gates before implementation.
...
1. In `skills/spec-workflow/SKILL.md`, add `specs/<id>/GATES.json` as the required gate-state artifact once a task enters spec-driven workflow. Define the minimal schema exactly as:
...
2. In `skills/spec-workflow/SKILL.md`, add the shared state rules:
...
5. In `skills/spec-loop-runner/SKILL.md`, update prerequisites to require sibling `GATES.json` and both `product.status` and `tech.status` equal to `approved` before implementation starts. Keep the existing requirement that implementation remains bounded by the latest approved specs; because the schema has no hashes or revisions, this remains a workflow responsibility rather than an automatic check.
...
- B6-B7: Covered by product-phase update rules in `spec-workflow` and `spec-write-product`.
- B8: Covered by TECH prerequisites in `spec-write-tech`.
- B9-B10: Covered by TECH update and approval rules in `spec-workflow` and `spec-write-tech`.
- B11: Covered by implementation prerequisites in `spec-loop-runner`.
...
- Run `jq . specs/02-feature-gate-status-json/GATES.json` to verify the example gate state file is valid JSON.
- Run `rg "GATES\\.json|product\\.status|tech\\.status|pending|approved" skills specs/02-feature-gate-status-json` to confirm all relevant skill docs and this spec mention the new gate-state contract.
- Review `skills/spec-workflow/SKILL.md`, `skills/spec-write-product/SKILL.md`, `skills/spec-write-tech/SKILL.md`, and `skills/spec-loop-runner/SKILL.md` to ensure no references introduce `sha256`, `reviewedRevision`, timestamps, approvers, or multi-artifact support.
